...
def _default_output_root(input_root: Path) -> Path:
    return input_root.with_name(f"{input_root.name}_patch")
...
def _load_frame_stack(frames, key: str, frame_count: int, image_size: int):
...
def _scene_complete(meta_path: Path, image_dir: Path, background_dir: Path, patch_count: int) -> bool:
    if not meta_path.is_file():
        return False
    for patch_id in range(patch_count):
        name = f"{patch_id:03d}.npy"
        if not (image_dir / name).is_file() or not (background_dir / name).is_file():
            return False
    return True
...
def _pack_scene(
    ...
    image_dir = split_out / "patches" / "images" / scene_name
    background_dir = split_out / "patches" / "backgrounds" / scene_name

    if not overwrite and _scene_complete(out_meta_path, image_dir, background_dir, patch_count):
    ...
    for gy in range(grid):
        ...
        for gx in range(grid):
            ...
            x1 = x0 + patch_size
            patch_id = gy * grid + gx
            patch_name = f"{patch_id:03d}.npy"
            image_path = image_dir / patch_name
            background_path = background_dir / patch_name

            np.save(image_path, np.ascontiguousarray(image_stack[:, y0:y1, x0:x1, :]))
            np.save(background_path, np.ascontiguousarray(background_stack[:, y0:y1, x0:x1, :]))
            patches.append(
                {
                    "patch_id": patch_id,
                    "patch_y": y0,
    ...
    out_meta = {
        ...
        "voxel_path": meta.get("voxel_path"),
...
def parse_args():
    parser = argparse.ArgumentParser(description="Pack ODT MAE 16x16 patches into per-scene patch npy files.")
    ...
    parser.add_argument("--num-processes", type=int, default=16, help="Worker processes. Use >1 for multiprocessing.")
    parser.add_argument("--chunk-size", type=int, default=8, help="Scenes per multiprocessing task chunk.")
    parser.add_argument("--print-every", type=int, default=None, help=argparse.SUPPRESS)
