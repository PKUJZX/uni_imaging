...
def init_config():
    ...
    for item in process_overrides(args.overrides):
        if "=" not in item:
            continue
...
def init_distributed(seed=42):
    ...
    device = torch.device(f"cuda:{local_rank}")##当前节点的第几个GPU
    torch.cuda.set_device(device)
    
    # Set random seeds
    # Each process gets a different seed derived from the base seed
    process_seed = seed + global_rank
    torch.manual_seed(process_seed)##cpu种子
    torch.cuda.manual_seed(process_seed)##gpu种子
    torch.cuda.manual_seed_all(process_seed) 
    np.random.seed(process_seed)##numpy种子
